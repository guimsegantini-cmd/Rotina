    ...
    refeicoes: Schema.array({
      items: Schema.object({
        properties: {
          ...
          ingredientes: Schema.array({ items: Schema.string() }),
        },
      }),
    }),
  },
...
- "ingredientes": lista curta (até 6) dos ingredientes/alimentos principais
  dessa refeição, em português, cada um só o nome do alimento sem
  quantidade nem unidade de medida (ex: "ovo", "queijo", "banana",
  "aveia") — usada para sugerir receitas alternativas com os mesmos
  ingredientes, então prefira o nome genérico do alimento em vez de marcas
  ou preparo específico.
...
export async function analisarPlanoAlimentar(dataUrl) {
  ...
  const paraItem = (r) => ({
    ...
    ingredientes: Array.isArray(r.ingredientes) ? r.ingredientes.filter(Boolean) : [],
    notificar: true,
  });
